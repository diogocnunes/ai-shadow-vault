...
assert_contains "### Branches and worktrees" "$AGENTS_FILE"
assert_contains 'Protected branches are `main`, `develop`, `stage`, `release/*`, and `hotfix/*`.' "$AGENTS_FILE"
assert_contains 'create a sibling worktree from the current `HEAD`' "$AGENTS_FILE"
assert_contains '`<username>/task-<number>-<slug>`' "$AGENTS_FILE"
assert_contains '`git config gitlab.pdmfc.com.username`' "$AGENTS_FILE"
assert_contains 'If none is provided, use `000`.' "$AGENTS_FILE"
assert_contains "### Commit and delivery workflow" "$AGENTS_FILE"
assert_contains "Commit only after the review is green" "$AGENTS_FILE"
assert_contains "Use Conventional Commits" "$AGENTS_FILE"
assert_contains 'use `#000` when the plan has no task number' "$AGENTS_FILE"
assert_contains 'Never include `Co-authored-by`, `Generated-by`' "$AGENTS_FILE"
assert_contains 'rtk git push -u <remote> HEAD' "$AGENTS_FILE"
assert_contains 'GitHub Pull Request with `gh pr create`' "$AGENTS_FILE"
assert_contains 'GitLab Merge Request with `glab mr create`' "$AGENTS_FILE"
assert_contains "Never merge automatically." "$AGENTS_FILE"
assert_not_contains "Without explicit user authorization in the current conversation" "$AGENTS_FILE"
assert_not_contains "Never commit automatically." "$AGENTS_FILE"
...
assert_contains "enforced deterministically where possible in ~/.claude/settings.json" "$CLAUDE_FILE"
assert_contains "authoritative for protected-branch pushes and commit-message checks" "$CLAUDE_FILE"
assert_contains "AGENTS.md remains authoritative for workflow decisions" "$CLAUDE_FILE"
...
assert_contains "enforced deterministically where possible in ~/.claude/settings.json" "$CLAUDE_NS_PROJECT_DIR/CLAUDE.md"
